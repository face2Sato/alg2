...
/*downheapでは、kの初めの値はヒープ構造を作る配列の先頭のインデックス*/
...
void downheap(int *A, int k, int n) { // downheapによってヒープチェック・メンテナンスを行う関数
    ...
    while(k <= n/2) {
        ...
        if(j < n && A[j]>A[j+1]/* Completed*/) j++; //２つの子ノードのうち、どちらと比較すればいいか
        if(v <= A[j]/* Completed */) break;
...
void insert(int *A, int v, int n) { //挿入（引数：　ヒープの配列、新規挿入する値、挿入後のデータ数）
    A[n] = v;
    upheap(A,n);
    /* Completed ＜upheap:insert関数は配列の末尾に挿入したい値を置く。その為、葉のほうから上に要素を入れ替えていく操作を行うupheapと併せて用いる＞*/
}

void replace(int *A, int v, int n) { //根の値の置き換え（引数：　ヒープの配列、置き換える値、データ数）
    A[1] = v;
    downheap(A,1,n);
    /* Completed ＜downheap:replace関数は配列の先頭に挿入したい値を置く。その為、親のほうから下に要素を入れ替えていく操作を行うdownheapと併せて用いる＞*/
}
...
int delete_root(int *A,
    ...
    downheap(A, 1, *n);
    /* Completed 
    ＜downheap:ルートをヒープの末尾にある子ノードに置き換えており、それからヒープ構造として再構築することでルートの削除を実装する。その為には配列の先頭に挿入する値を持ってくるdownheapを用いる＞*/
...
void myheapsort(int *A, int n) {
    ...
    for(i = n; i >= 2; i--) {
        ...
        downheap(A, 1, i-1);
        /* Completed!! ＜downheap:for文のiがn to 2であるから、ヒープ構造の末尾にある要素をdownheapにかけていくことでソートする方針だと読み取ったため＞*/
    }
}
