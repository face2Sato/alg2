...
int main(){
    ...
    for(i = 0; i < SIZE; i++) index[i]=0; // Initialize ( indexは256個の配列。以下の行で現れた文字の「文字コードに対応する添字番号」の配列要素をカウントする )
    ...
    while(i>1){
        ...
        /* Pick up the 2 minimum appearance characters (delete root node twice)*/
...
void downheap(Node *A, int k, int n){
    ...
    while (k<=n/2) { 
        ...
        if (j<n){
            if( A[j].count > A[j+1].count || (A[j].count == A[j+1].count && A[j].word > A[j+1].word )) j++; //ヒープ条件の確認（Ex02とほぼ同じ)
        }

        if ( v.count < A[j].count || (v.count == A[j].count && v.word > A[j].word )) break;
...
NodePointer make_1node(Node keyA){
  NodePointer n;
...
int Huffman_DFS(NodePointer p,char *bin){
    ...
    while(1){
        ...
        if ( p->vertex.visited == 0 ) {
            if( p->vertex.articulation != -1 ) printf("%x [%3d]::  Left:%x [%3d]  Right:%x [%3d]\n",p,p->vertex.count,p->c_left,p->c_left->vertex.count,p->c_right,p->c_right->vertex.count);
            else{
                strcpy(p->vertex.bincode,bin);
                p->vertex.bits = strlen(p->vertex.bincode)*p->vertex.count;
                printf("%x [%3d]: <Terminal> %c  Code: %8s (%2dbits)\n",p,p->vertex.count,p->vertex.word,p->vertex.bincode,p->vertex.bits);
